        ...
        async for message in websocket:        
            try:
                ...
                if message_type == MessageType.USER:
                    ...
                    hop_message = HopMessage(MessageType.HOP, transaction_type, hops[1], PRIORITY.HOP_1.value)
                ...
                if message_type == MessageType.FORWARD:
                    ...
                    hop_message = ForwardMessage(message_type=MessageType.FORWARD, transaction_type=transaction_type, 
                        target_server=target_server, transaction_id=transaction_id, hop_id=hop_id, origin_server=origin_server, data=data)
                ...
                if message_type == MessageType.BACKWARD:
                    ...
                    hop = history_table.transactions.get(transaction_id).hops.get(hop_id)
                    hop.end_websocket_transmission(convert_to_datetime(message_data['websocket_receive_time']))
                    hop.start_websocket_transmission(convert_to_datetime(message_data['websocket_send_time']))
...
async  def insert_nexthop_to_priority_queue(transaction_id, hop_id,client_id):
    ...
    if transaction:
        ...
        if hop_2 and hop_2.status == 'Active':
            hop_message = HopMessage(MessageType.HOP, transaction.transaction_type, hop_2, PRIORITY.HOP_2.value)
            await priority_queue.put((client_id, hop_message))
        ...
        try:
            ...
            if message_type == MessageType.BACKWARD: 
                ...
                '''RESER LOGIC'''
                    ...
                    hop = history_table.transactions.get(message.transaction_id).hops.get(message.hop_id)
                    forward_message = ForwardMessage( message_type=MessageType.FORWARD, transaction_type=transaction_type, target_server=ORDER_SERVER_PORT, transaction_id=message.transaction_id, hop_id=message.hop_id, origin_server=SERVER_PORT, data=hop.data)
                ...
                if message.hop_id == 1:
                    await insert_nexthop_to_priority_queue(message.transaction_id, 2, client_id)
            ...
            if message_type == MessageType.FORWARD:
                ...
                backward_message = BackwardMessage(message_type=MessageType.BACKWARD, transaction_type=transaction_type, 
                    transaction_id=message.transaction_id, hop_id=message.hop_id,origin_server=message.target_server, 
                    target_server=message.origin_server, result=result, websocket_receive_time=websocket_receive_time,
                    websocket_send_time=datetime.now(),queue_tracker=message.queue_tracker)
            ...
            if message_type == MessageType.HOP: 
                ...
                if transaction_type in [ TransactionType.T3, TransactionType.T4] :
                    if message.hop.hop_id ==1:
                        result = await execute_hop(db, message.hop.transaction_id, message.hop.hop_id)
                        if result:
                            await insert_nexthop_to_priority_queue(message.hop.transaction_id, 2, client_id)
                        else:
                            message.priority += 1
                    ...
                    if  message.hop.hop_id ==2:
                        ...
                        #get the current hop info and update the websocket_tracker timestamp
                        hop = history_table.transactions.get(message.hop.transaction_id).hops.get(message.hop.hop_id)
                        hop.start_websocket_transmission()
